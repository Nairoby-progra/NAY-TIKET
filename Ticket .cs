...
if (objTecnico == null)
{
throw new InvalidOperationException("No se puede asignar un tecnico
nulo.");
}
if (!objTecnico.puedeAtender(strCategoria))
{
throw new InvalidOperationException("El tecnico no esta disponible
o no atiende esta categoria.");
}
...
public void registrarError(string strTipo, string strDescripcion, string
strImpacto)
{
string strError = $"{strTipo}: {strDescripcion} | Impacto:
{strImpacto}";
...
{
throw new InvalidOperationException("Solo se puede resolver un
ticket asignado.");
}
strEstado = "Resuelto";
...
{
throw new InvalidOperationException("Solo se puede cerrar un ticket
resuelto.");
}
strEstado = "Cerrado";
...
{
blnEscalado = true;
if (!strPrioridad.Equals("Critica",
...
{
Console.WriteLine($"#{intNumero} | {strTitulo} | Estado: {strEstado} |
Prioridad: {strPrioridad} | Categoria: {strCategoria}");
Console.WriteLine($" Solicitante: {objSolicitante.strNombre}");
Console.WriteLine($" Tecnico: {(objTecnicoAsignado != null ?
objTecnicoAsignado.strNombre : "Sin asignar")} | Escalado: {blnEscalado}");
}
